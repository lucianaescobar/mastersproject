...
```{r}
nepsyeicPrePost <- read_excel("Desktop/R/PretermControl/Neuropsych/nepsyeicpretermcontrol.xlsx")
   

str(nepsyeicPrePost)

nepsyeicPrePost$Status <- as.factor(nepsyeicPrePost$Status)
...
nepsyeicPrePost$Status <- relevel(nepsyeicPrePost$Status, ref= "Preterm")
...
table_one <- tableby(Status ~ ., data= nepsyeicPrePost)
...
model <- lmer(TN ~ Status + (1 | Subject) + Age, data = nepsyeicPrePost, REML = FALSE)
...
storage <- list()
for(i in names(nepsyeicPrePost)[-c(1:3)]){
  storage[[i]] <- lm(get(i) ~ Status, data= nepsyeicPrePost)
}
...
stor <- list()
stor2 <- list()
for(i in names(nepsyeicPrePost)[-c(1:2)]) {
 stor[[i]] <- aov(as.formula(paste(i, "~ Status")), data= nepsyeicPrePost) 
